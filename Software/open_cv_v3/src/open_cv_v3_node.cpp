...
ardrone_autonomy::Navdata nav_in;  
  ...
  //ServiceClient servicesetcamchannel;
...
//-----------------------------------------------          P CONTROLLER MODULE         -------------------------------------------------//
     ...
     // cmdy=  -0.05+ ((0.05+0.05)/ (180+180))*(tempy+180);
      ...
      cmdy=  -speed_limit+ ((speed_limit+speed_limit)/ (180+180))*(tempy+180);
            ...
            derivativex=(errorx-prev_errorx)/(float)dt;
            derivativey=(errory-prev_errory)/(float)dt;
...
begin = ros::Time::now().toSec();
       ...
       else if(tempx>-40 && tempx<40 && tempy>-30 && tempy<30)
        {
             stop();
             pub_twist.publish(twist_msg_hover);
